...
function echarts_2(chart) {
    ...
    function updateChart(numFields) {
        ...
        fetch('http://localhost:8080/AnswerQuality/get' + numFields + 'InfluenceQuality')
            .then(response => response.json())  // 解析 JSON 格式的响应
            .then(data_get => {
                // 显示加载动画
                document.querySelector('.loading').style.display = 'none';

                const dataArray = data_get.map(item => {
                    const key = Object.keys(item)[0]; // 提取字段名
                    const value = Number(Object.values(item)[0]); // 转换为数字
                    return { key, value };
                });
    ...
    function updateChart(numFields) {
        ...
        fetch('http://localhost:8080/AnswerQuality/get' + numFields + 'InfluenceQuality')
            ...
            .then(data_get => {
                ...
                const yAxisData = dataArray.map(item => item.key); // 按排序后的顺序提取字段名
                const whiteBoxData = dataArray.map(item => item.value); // 按排序后的顺序提取数值
                const seriesData = whiteBoxData.map(value => value.toFixed(2)); // 计算 seriesData
